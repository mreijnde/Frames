classdef (SharedTestFixtures = {matlab.unittest.fixtures.PathFixture('../../../')} ) ...
    ...
    methods(Test)
        ...
        function splitAggregateTest(t)
            ...
            s2 = series.split(g1).aggregate(@(x) sum(x), 2, 'applyToData');
            s3 = series.split(g1).aggregate(@(x) x.sum(2));
            t.verifyEqual(s1,frames.DataFrame([8 2],[],["grp2" "grp1"]).asRowSeries())
            t.verifyEqual(s1,s2)
            t.verifyEqual(s1,s3)
            ...
            t.verifyEqual(s4,frames.DataFrame([1 5 5;4 11 11],[],["Group1" "Group2"]))
